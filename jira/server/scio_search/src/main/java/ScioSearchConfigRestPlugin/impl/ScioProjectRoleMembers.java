...
public class ScioProjectRoleMembers {
  ...
  public ProjectRoleMembersResponse getProjectRoleMembers(
      @QueryParam("projectId") String projectId, @QueryParam("roleId") String roleId) {
    ...
    if (projectRole == null) {
      throw new NotFoundException(
          String.format("Project Role %s not found for project %s", roleId, projectId));
    }
    ProjectRoleActors projectRoleActors =
        projectRoleManager.getProjectRoleActors(projectRole, project);
    if (projectRoleActors == null) {
      throw new NotFoundException(
          String.format(
              "Project Role Actors not found for project %s, role %s", projectId, roleId));
    }
    ...
    for (RoleActor actor : roleActors) {
      ProjectRoleMembersResponse.JiraRoleActorInfo actorInfo =
          new ProjectRoleMembersResponse.JiraRoleActorInfo();
      actorInfo.id = actor.getId().toString();
      actorInfo.displayName = actor.getDescriptor();
      actorInfo.type = actor.getType();
      // Jira's api call returns userName as actor name whereas actor.getParameter is the userKey so
      // we need to get the username from the usermanager
