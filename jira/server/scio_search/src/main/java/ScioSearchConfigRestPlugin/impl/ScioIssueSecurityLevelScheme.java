...
/**
 * Equivalent api call:
 * https://developer.atlassian.com/cloud/jira/platform/rest/v3/api-group-project-permission-schemes/#api-rest-api-3-project-projectkeyorid-issuesecuritylevelscheme-get
 * ProjectManager Doc:
 * https://docs.atlassian.com/software/jira/docs/api/7.6.1/com/atlassian/jira/project/ProjectManager.html
 * IssueSecuritySchemeManager Doc:
 * https://docs.atlassian.com/software/jira/docs/api/8.4.1/com/atlassian/jira/issue/security/IssueSecuritySchemeManager.html
 */
@Named
...
public class ScioIssueSecurityLevelScheme {
  ...
  @Inject
  public ScioIssueSecurityLevelScheme(
  ...
  public IssueSecuritySchemeResponse getIssueSecuritySchemeMembers(
      @QueryParam("projectId") String projectId) {
    Utils.validateUser(userManager, pluginSettingsFactory.createGlobalSettings());

    Project project =
        ComponentAccessor.getProjectManager().getProjectObj(Long.parseLong(projectId));
    if (project == null) {
    ...
    if (scheme == null) {
      ...
      throw new NotFoundException(
          String.format("Security level for project %s does not exist", projectId));
    }
    IssueSecurityLevelScheme schemeObject =
        ComponentAccessor.getComponentOfType(IssueSecuritySchemeManager.class)
            .getIssueSecurityLevelScheme(scheme.getId());
    List<IssueSecurityLevel> securityLevels =
        ComponentAccessor.getIssueSecurityLevelManager().getIssueSecurityLevels(scheme.getId());
    ...
    for (IssueSecurityLevel securityLevel : securityLevels) {
      ...
      IssueSecuritySchemeResponse.IssueSecurityLevel level =
          new IssueSecuritySchemeResponse.IssueSecurityLevel();
